                ...
                <div class="card-body">
                        ...
                        <form action="{{ route($profileData->access_level.'.location.lga.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="col-sm-5">
                                    <select class="form-control @error('senatorial_district_id') is-invalid @enderror" name="senatorial_district_id" id="senatorial_district_id" data-boundary="state">
                                        ...
                                        @foreach ($senatorialDistricts as $district)
                                            <option value="{{ $district->id }}" data-state-id="{{ $district->state_id }}" {{ old('senatorial_district_id') == $district->id ? 'selected' : '' }}>{{ $district->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-sm-5">
                                    <select class="form-control @error('federal_constituency_id') is-invalid @enderror" name="federal_constituency_id" id="federal_constituency_id" data-boundary="state">
                                        ...
                                        @foreach ($federalConstituencies as $constituency)
                                            <option value="{{ $constituency->id }}" data-state-id="{{ $constituency->state_id }}" {{ old('federal_constituency_id') == $constituency->id ? 'selected' : '' }}>{{ $constituency->name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="alert alert-info">
                                Manual Add LGA lets you map each LGA correctly while creating it. Bulk Import adds LGAs but does not automatically create Senatorial District or Federal Constituency mappings. For complete electoral geography with all relationships automatically configured, use QuickStart Geography Provisioning.
