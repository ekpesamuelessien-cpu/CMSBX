              ...
              <div class="card-body">
                <div class="tab-content">
                  ...
                  <form class="form-horizontal" id="profileData" method="POST" action="{{ route($profileData->access_level.'.account.complete-profile.store') }}" enctype="multipart/form-data">
                        ...
                        @unless($profileData->access_level === 'nationaladmin')
                        ...
                        @foreach($pollingUnits as $pu)
                            @if($pu->ward_id == $profileData->ward_id)
                                @if(empty($profileData->polling_unit_id))
                                    <option  value="{{ $pu->id }}" {{ ($profileData->polling_unit_id == $pu->id) ? 'selected' : '' }}>{{ $pu->name }}</option>
                                @else
                                <option readonly  value="{{ $pu->id }}" {{ ($profileData->polling_unit_id == $pu->id) ? 'selected' : '' }}>{{ $pu->name }}</option>
                                @endif
                            @endif
                        @endforeach
                        </select>
