            ...
            <div class="col-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Edit Federal Constituency</h3>
                    ...
                    <div class="card-body">
                        <form action="{{ route($profileData->access_level.'.location.federal-constituency.update', $constituency->uuid) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('POST')
                            <div class="form-group row">
                                <label for="name" class="col-sm-3 col-form-label">Federal Constituency Name</label>
                                <div class="col-sm-7">
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ old('name', $constituency->name) }}">
                                    @error('name')
                                ...
                                <div class="col-sm-5">
                                    <select class="form-control @error('state_id') is-invalid @enderror" name="state_id" id="state_id">
                                        ...
                                        @foreach ($states as $state)
                                            <option value="{{ $state->id }}" {{ old('state_id', $constituency->state_id) == $state->id ? 'selected' : '' }}>{{ $state->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('state_id')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-sm-5">
                                    <select class="form-control @error('senatorial_district_id') is-invalid @enderror" name="senatorial_district_id" id="senatorial_district_id">
                                        ...
                                        @foreach ($senatorialDistricts as $district)
                                            <option value="{{ $district->id }}" {{ old('senatorial_district_id', $constituency->senatorial_district_id) == $district->id ? 'selected' : '' }}>{{ $district->name }}{{ $district->state ? ' - '.$district->state->name : '' }}</option>
                                        @endforeach
                                    </select>
