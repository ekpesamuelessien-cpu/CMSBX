        ...
        <div class="container-fluid">
            ...
            <div class="col-12">
                ...
                <div class="card-header">
                    <h3 class="card-title">Edit Polling Unit</h3>
                ...
                <div class="card-body">
                        ...
                        <form action="{{ route($profileData->access_level.'.location.pu.update', $pu->uuid) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('POST')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-7">
                                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ old('name', $pu->name) }}">
                                    @error('name')
                                        ...
                                        @foreach ($wards->groupBy('lga_name') as $lgaName => $wardsByLga)
                                            <optgroup label="{{ $lgaName }}">
                                                @foreach ($wardsByLga as $ward)
                                                    <option value="{{ $ward->id }}" {{ (old('ward_id', $pu->ward_id) == $ward->id) ? 'selected' : '' }}>{{ $ward->name }}</option>
                                                @endforeach
                                            </optgroup>
                                        @endforeach
