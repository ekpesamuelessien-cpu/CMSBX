...
class LgaAdminController extends Controller
{
    ...
    public function lgaAdminDashBoard()
{
    $pageTitle    = 'LGA Members/Agents Dashboard';
    $profileData  = $this->getProfileData();

    if ($profileData->access_level == 'lgaadmin') {
        $lga = LocalGovernmentArea::where('id', $profileData->lga_id)->first();
    } else {
        abort(403);
    }

    // Get the total count of wards in the LGA
    $wardCount = Ward::where('lga_id', $lga->id)->count();

    // Get the count of wards that have associated users
    $wardWithUsersCount = Ward::where('lga_id', $lga->id)
        ->whereHas('users') // Ensures only wards with users are counted
        ->count();

    // Get all the ward IDs in the LGA
    $wards = Ward::where('lga_id', $lga->id)->pluck('id')->toArray();

    // Get the total count of polling units in the wards
    $puCount = PollingUnit::whereIn('ward_id', $wards)->count();

    // Get the count of polling units that have associated users
    ...
    public function lgaadminProfile()
    {
            $profileData = $this->getProfileData();
            $pageTitle = 'Profile';
      ...
      public function lgaadminProfileStore(Request $request)
      {
          $profileData = $this->getProfileData();
    ...
    public function lgaadminChangePassword()
    {
    ...
    public function lgaadminUpdatePassword(Request $request)
    {
    ...
    public function lgaadminLogout(){
        Auth::logout();
